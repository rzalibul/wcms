<?php
		...
		if($conn)
		{
			...
			if($result)
			{
				...
				if($userid != $_SESSION['userid'])
				{
					...
					fwrite($stream, PHP_EOL . "<".$timestamp.">Access denied for deletion! User IDs (Creator: ".$_SESSION['userid'].", executor: ".$userid.") don't match - IP: ".empty($_SERVER['HTTP_X_FORWARDED_FOR']) ? $_SERVER['REMOTE_ADDR'] : $_SERVER['HTTP_X_FORWARDED_FOR']);
					fclose($stream);
					echo "Access denied!";
					exit();
				}
	...
	if(isset($_POST['contentid']))
	{
		...
		if($conn)
		{
			...
			$sql = "INSERT INTO content_archive SELECT * FROM content WHERE contentID='$contentid';";
			if(mysqli_query($conn, $sql))
			{
				...
				{
					$timestamp = date("Y-m-d H:i:s");
					fwrite($stream, PHP_EOL . "<".$timestamp.">Article removed by user ".$_SESSION['username']." (User ID: ".$_SESSION['userid']." )");
					fclose($stream);
				...
				{
					$timestamp = date("Y-m-d H:i:s");
					fwrite($stream, PHP_EOL . "<".$timestamp.">Deletion unsuccessful - SQL query failed ".mysqli_error($conn));
					fclose($stream);
